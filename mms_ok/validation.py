...
def get_block_pipe_constraints(
    ...
    interface = _normalize_device_interface(device_interface)
    speed = _normalize_usb_speed(usb_speed)

    if interface == "USB2":
        speed_max = {"FULL": 64, "HIGH": 1024}.get(speed)
        if speed_max is not None:
            max_allowed = speed_max
        elif max_block_size > 0:
            max_allowed = max_block_size
        else:
            max_allowed = 64
    ...
    max_allowed = 16384 if max_block_size <= 0 else min(max_block_size, 16384)
    return BlockPipeConstraints(
        min_block_size=16,
        max_block_size=max_allowed,
        block_size_multiple=16,
